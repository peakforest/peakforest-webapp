...
<script type="text/javascript">
	...
	loadSpectre<%=spectrumDivId %> = function() {
		lastPoint = "";
		$("#peakdivMSspectrum").html("");
		$('#containerMSspectrum${spectrum_div_id}').highcharts(
						{
							chart : {
								zoomType : 'x',
								spacingRight : 10,
							...
							tooltip : {
								...
								formatter : function() {
									if ((this.series.name).match(/ - HIDE$/)) {
										...
										var compo = '';
										var showCompoAttrib = false;
										<% for (int i = 0; i < seriesShow.length; i++) { %>
											if (<%=showCompoAdducts[i] %> && this.series.index == <%=(seriesShow.length+i) %>) {
												compo = seriesCompo<%=spectrumDivId %><%=i %>[this.x];
												attrib = seriesAdducts<%=spectrumDivId %><%=i %>[this.x];
												//if (attrib == "null") { attrib = ""; } 
												showCompoAttrib = true;
											}
										<% } %>
...
// 											serieSymbole = 0;
									...
									for (int i = 0; i < seriesShow.length; i++) {
										...
										out.print("marker : {symbol : seriesSymboles["+serieSymbole+"], enabled : true, "
												+ "lineColor : seriesColors["+serieColor+"], lineWidth : 1" + (showTinySymbols[i] ? ", radius : 2" : "")
												+ "}, ");
		...
		chart<%=spectrumDivId %> = $('#containerMSspectrum${spectrum_div_id}').highcharts();
	...
	selectPoint = function(s, x, y, spectrum, id) {
		...
		if (i > -1 && lastPoint != newPoint) {
			...
			$("#peakdivMSspectrum").html(divConent);
			lastPoint = newPoint;
		} else {
			$("#peakdivMSspectrum").html("");
			lastPoint = "";
...
// 		var spectraCode = {};
		...
		if (serie<%=spectrumDivId %><%=(i) %>IsShow) {
			// meta
			var keyID = seriesMetadata<%=spectrumDivId %><%=(i) %>['pforest_id'];
			var keyPfemName = seriesMetadata<%=spectrumDivId %><%=(i) %>['pfem_name'];
		...
		// reload
		$("#metadataMS_names<%=spectrumDivId %>").html(spectraNameString);
// 		$("#metadataMS_codes<%=spectrumDivId %>").html(spectraNameString);
		$("#metadataMS_rt<%=spectrumDivId %>").html(spectraRTString);
		$("#metadataMS_polarity<%=spectrumDivId %>").html(spectraPolarityString);
		$("#metadataMS_ionization<%=spectrumDivId %>").html(spectraIonizationString);
		
		$("#metadataMS_type<%=spectrumDivId %>").html(spectraTypeString);
		$("#metadataMS_authors<%=spectrumDivId %>").html(spectraAuthorsString);
		$("#metadataMS_owners<%=spectrumDivId %>").html(spectraOwnersString);
		$("#metadataMS_copyright<%=spectrumDivId %>").html(spectraCopyrightsString);
	}; // 
	loadMetadataDetail<%=spectrumDivId %>();
