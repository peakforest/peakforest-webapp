...
<button class="btn btn-xs" onclick="Jmol.saveImage(molviewer,'png');"><i class="fa fa-save"></i></button>
<button class="btn btn-xs" onclick="spinMol()"><i id="molviewer-spin" class="fa fa-refresh"></i></button>
<button class="btn btn-xs" onclick="vibrMol()"><i id="molviewer-vibr" class="fa fa-spinner"></i></button>
	<br /><spring:message code="addon.jsMol.poweredBy" text="<small>powered by <a href='http://www.jmol.org' target='_blank'>JSmol</a>.</small>" />
</div>
...
$(document).ready(function(){
	setTimeout(function() {	
		var xxxx = "${mol_nb_clean_name}";
		var	script = 'set language "en"; set errorCallback "myCallback";'
	...
	<%
	String molName2 = request.getAttribute("mol_nb_clean_name").toString().replaceAll("\"", "''");
	request.setAttribute("mol_nb_clean_name2", molName2);
	%>
		molviewer = Jmol.getApplet("molviewer", Info);
		setTimeout(function(){
			Jmol.loadFile(molviewer, "numbered/${inchikey}.mol");
			// <c:forEach var="mol_nb_3D_script" items="${mol_nb_3D_scripts}">
			Jmol.script(molviewer,"${mol_nb_3D_script}"); // </c:forEach>
			Jmol.script(molviewer,"set echo top center;echo \"${mol_nb_clean_name2}\"; refresh;");
