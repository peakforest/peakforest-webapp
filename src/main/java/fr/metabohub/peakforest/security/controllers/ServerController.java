...
public class ServerController {
	...
	public @ResponseBody Object getServerLoad(HttpServletRequest request, HttpServletResponse response, Locale locale)
			throws IOException {
		...
		output.put("cpu_load",
				(double) operatingSystemMxBean.getSystemLoadAverage() / Runtime.getRuntime().availableProcessors());
		output.put("memory_load", ((float) memHeap.getUsed()) / ((float) memHeap.getMax()));
		return output;
	}

	@RequestMapping(value = "/call-gc", method = RequestMethod.POST)
	public @ResponseBody Object callGC(HttpServletRequest request, HttpServletResponse response, Locale locale) {
	...
	public @ResponseBody Object getDiskUsage(HttpServletRequest request, HttpServletResponse response, Locale locale)
			throws IOException {
		response.setHeader("Cache-Control", "max-age=0");
		...
		File generated = new File(PeakForestUtils.getBundleConfElement("generatedFiles.prefix") + File.separator
				+ PeakForestUtils.getBundleConfElement("generatedFiles.folder"));
		long totalSpaceGenerated = FileUtils.sizeOfDirectory(generated);
		File uploaded = new File(PeakForestUtils.getBundleConfElement("uploadedFiles.folder"));
	...
	public @ResponseBody String getLogFile(HttpServletRequest request, HttpServletResponse response, Locale locale) {
		// adminLog("called 'clean uploaded files' function ");

		String fileName = PeakForestUtils.getBundleConfElement("log.filename");
	...
	public @ResponseBody Object resetLogFile(HttpServletRequest request, HttpServletResponse response, Locale locale) {
